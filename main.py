...
def parallel_main(model_nums_list: list=None):
...
def not_parallel_main(is_debug: bool=False, model_nums_list: list=None):
    print(f'Start run in parallel: for each fold compare all the models')
    logging.info(f'Start run in parallel: for each fold compare all the models')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 4:
        ...
        if outer_model_nums_list != 'False':
            outer_model_nums_list = json.load(outer_model_nums_list)
        else:
    ...
    # read function
    if is_parallel:
        parallel_main(model_nums_list=outer_model_nums_list)
    else:
        # need to put line 96: @ray.remote in comment
        not_parallel_main(outer_is_debug, model_nums_list=outer_model_nums_list)
