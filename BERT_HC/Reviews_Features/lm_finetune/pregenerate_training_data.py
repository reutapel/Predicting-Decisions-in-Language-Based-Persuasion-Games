...
from bert_constants import BERT_PRETRAINED_MODEL, REVIEWS_FEATURES_PRETRAIN_DATA_DIR, MAX_SENTIMENT_SEQ_LENGTH, \
...
class DocumentDatabase:
    def __init__(self, reduce_memory=False):
        if reduce_memory:
            ...
            self.documents = []
            self.document_ids = []
            self.documents_feature_1_labels = []
            self.documents_feature_2_labels = []
            self.documents_feature_3_labels = []
            self.documents_feature_4_labels = []
    ...
    def add_document(self, unique_id, document, feature_1_label, feature_2_label, feature_3_label, feature_4_label):
        if not document:
            return
        if self.reduce_memory:
            ...
            self.documents_feature_1_labels.append(feature_1_label)
            self.documents_feature_2_labels.append(feature_2_label)
            self.documents_feature_3_labels.append(feature_3_label)
            self.documents_feature_4_labels.append(feature_4_label)
    ...
    def __getitem__(self, item):
        if self.reduce_memory:
            ...
            return self.document_ids[item], self.documents[item], self.documents_feature_1_labels[item],\
                   self.documents_feature_2_labels[item], self.documents_feature_3_labels[item],\
                   self.documents_feature_4_labels[item]
...
def create_instances_from_document(
    ...
    unique_id, document, feature_1_label, feature_2_label, feature_3_label, feature_4_label = doc_database[doc_idx]
    ...
    while num_masked < len(cand_indices) and num_masked < num_to_mask:
        ...
        instance = {
            ...
            "feature_1_label": str(feature_1_label),
            "feature_2_label": str(feature_2_label),
            "feature_3_label": str(feature_3_label),
            "feature_4_label": str(feature_4_label),
        }
...
def main(exp_num='1'):
    ...
    generate_data_for_domain(args, args.exp_num)


def generate_data_for_domain(args, exp_num):
    ...
    features_list = reviews_features_treat_dict[exp_num]["features_list"]
    features_list_columns = [f'{column}' for column in features_list]
    num_features = len(features_list)

    feature_1_column = features_list_columns[0]
    if num_features > 1:
        feature_2_column = features_list_columns[1]
    else:
        feature_2_column = features_list_columns[0]
    if num_features > 2:
        feature_3_column = features_list_columns[2]
    else:
        feature_3_column = features_list_columns[0]
    if num_features == 4:
        feature_4_column = features_list_columns[3]
    else:
        feature_4_column = features_list_columns[0]

    id_column = 'review_id'
    text_column = 'review'

    with DocumentDatabase(reduce_memory=args.reduce_memory) as docs:
        print(f"\nGenerating data for experiment number: {exp_num}")
        output_dir = Path(REVIEWS_FEATURES_PRETRAIN_DATA_DIR) / exp_num
        output_dir.mkdir(exist_ok=True, parents=True)
        unique_ids, reviews, feature_1_labels, feature_2_labels, feature_3_labels, feature_4_labels =\
        ...
        for dataset in ("train", "dev"):
            ...
            df = pd.read_csv(DATASET_FILE, header=0, encoding='utf-8',
                             usecols=[id_column, text_column] + features_list_columns).\
                set_index(keys=id_column, drop=False).sort_index()
            for feature in features_list_columns:
                df = df[df[feature].notnull()]
            unique_ids += df[id_column].astype(int).tolist()
            reviews += df[text_column].apply(tokenizer.tokenize).tolist()
            feature_1_labels += df[feature_1_column].astype(int).tolist()
            feature_2_labels += df[feature_2_column].astype(int).tolist()
            feature_3_labels += df[feature_3_column].astype(int).tolist()
            feature_4_labels += df[feature_4_column].astype(int).tolist()

        for unique_id, doc, feature_1_label, feature_2_label, feature_3_label, feature_4_label in\
                tqdm(zip(unique_ids, reviews, feature_1_labels, feature_2_labels, feature_3_labels, feature_4_labels)):
            if doc:
                # If the last doc didn't end on a newline, make sure it still gets added
                docs.add_document(unique_id, doc, feature_1_label, feature_2_label, feature_3_label, feature_4_label)
